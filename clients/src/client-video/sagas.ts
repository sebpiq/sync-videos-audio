...
import { FollowerConnectMessage, Message, TimeDiffResponseMessage, TYPE_WEBSOCKET_MESSAGE_FOLLOWER_CONNECT, TYPE_WEBSOCKET_MESSAGE_FOLLOWER_CONNECTED, TYPE_WEBSOCKET_MESSAGE_TIME_DIFF_QUERY, TYPE_WEBSOCKET_MESSAGE_TIME_DIFF_RESPONSE } from '../shared/websocket-messages'
...
function* sendTimeDiffQuerySaga(leaderTimestamp: number, followerId: ClientId) {
    yield call(websocket.send, followerId, {
        type: TYPE_WEBSOCKET_MESSAGE_TIME_DIFF_QUERY,
        payload: { leaderTimestamp },
    })
    const { timeDiffResponse, timeout } = yield race({
        timeDiffResponse: take(TYPE_WEBSOCKET_MESSAGE_TIME_DIFF_RESPONSE as Message['type']),
...
function* newFollowerSaga(action: FollowerConnectMessage) {
    ...
    yield call(websocket.send, action.payload.clientId, {
        type: TYPE_WEBSOCKET_MESSAGE_FOLLOWER_CONNECTED,
        payload: { timeDiff },
    })
}
...
function* rootSaga() {
    yield all([
        takeEveryWebsocketMessage(
            TYPE_WEBSOCKET_MESSAGE_FOLLOWER_CONNECT,
            newFollowerSaga
        ),
    ])
}
